<template>
    ...
    >
    <ul
        v-if="!isLoading && usersOfProject.length > 0"
        class="flex flex-col sm:grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4"
    >
        <li v-for="user in usersOfProject" :key="user._id">
            <UCard>
    ...
    <div v-else class="flex flex-col gap-4 justify-center items-center">
        ...
        <UButton @click="addUser">Dodaj członka</UButton>
    </div>
</template>
...
const usersOfProject = computed(() => {
    return users.value.filter(user =>
        user.name.toLowerCase().includes(searchStore.getSearch.toLowerCase())
    );
});
